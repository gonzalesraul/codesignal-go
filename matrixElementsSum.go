...
/*
MatrixElementsSum : The total price of all the rooms that are suitable for the CodeBots to live in
...
func MatrixElementsSum(matrix [][]int) int {
	...
	for col := 0; col < len(matrix[0]); col++ { //iterate over column (y) instead of rows (x)
		for row := 0; row < len(matrix); row++ {
			if matrix[row][col] == 0 { //"haunted room" find, skip the rooms below (x)
				break
			}
			sum += matrix[row][col]
		}
